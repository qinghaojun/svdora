...
def test_hendrycks_math(model, data_path, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1, gpu_memory_utilization=0.3):
    hendrycks_math_ins = []
    hendrycks_math_answers = []
    problem_prompt = (
        "Below is an instruction that describes a task. "
        "Write a response that appropriately completes the request,following clear and concise reasoning.Avoid repeating same sentence .Every response should end with the formality:'The answer is:' \n\n"
    ...
    llm = LLM(model=model, tensor_parallel_size=tensor_parallel_size, gpu_memory_utilization=gpu_memory_utilization)  # 设置 gpu_memory_utilization 参数
    res_completions = []
...
def parse_args():
    ...
    parser.add_argument("--gpu_memory_utilization", type=float, default=0.5)  # 添加显存利用率参数
    return parser.parse_args()

if __name__ == "__main__":
    args = parse_args()
    test_hendrycks_math(model=args.model, data_path=args.data_file, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size, gpu_memory_utilization=args.gpu_memory_utilization)
